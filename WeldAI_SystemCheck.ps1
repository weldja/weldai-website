...
elseif ($ramGB -ge 4)  { Warn "$ramGB GB — below minimum. System may be slow/unstable. Upgrade to 8 GB+ before installation." }
...
Head "10. Docker Engine & Image Pull Test"
...
foreach ($port in @(80, 443, 8501)) {
    ...
    if ($port -eq 80 -or $port -eq 443) {
        if ($inUse) { Warn "Port $port is already in use — another service (e.g. IIS) may conflict with Weld AI" }
        else        { Ok  "Port $port is free" }
    } else {
        if ($inUse) { Warn "Port $port is already in use — this is the Weld AI web interface port and must be free" }
        else        { Ok  "Port $port is free" }
    }
}
